...
if fileExists ==0
    msg = strcat('file does not exist in directory:  ', EEMfilename)
    error(msg)
end

fileID=fopen(EEMfilename); EEMlist=textscan(fileID,'%s %s %s %s %s %s', 'Delimiter', '\t'); fclose(fileID); % reads in the list of samples, tabs only so no whitespace issue - Sean
Rowcount=size(EEMlist{1},1);  % Error Catch - Sean
disp(Rowcount); % Error Catch - Sean
celldisp(EEMlist); %Error Catch - Sean
