...
# EMstep = There is an EmStep distance between each emissign value (rows). This is dictated by the number of pixels assigned to integrate 4.65nm = 8 pixels.
...
correct_EEMs <- function(data_directory, 
  ...
  # 3. Write out processed sample EEM matrices -----------------------------
    ...
    if(!dir.exists(blank_path)){
      ...
      dir.create(blank_path, recursive = T)
    }
...
process_aqualog <- function(data_directory,
  ...
  # allow for custom sample sheet to be specified (e.g. for skipping blanks)
  if(is.null(org_file)){
    ...
    # write out sample sheet in same place as default sample sheet
    sample_sheet_file = paste0(run_name, "_sample_sheet.csv")
    writeLines(paste0("writing out sample_sheet as: ", sample_sheet_file))
    write.csv(sample_sheet,
              file.path(out_dir,"processed_data",sample_sheet_file),
              row.names = F)
...
# function for reading flat eem csvs from directory
...
read_processed_data = function(run_out_dir){
  ...
  if(!any(run_indices$UniqueID %in% run_sheet$UniqueID)){
    stop(
...
compile_runs <- function(run_dirs,
  ...
  # read runs
  processed_runs = lapply(run_dirs, 
             function(x){
               tryCatch(expr = {read_processed_data(x)},
                        error = function(e) {
                          print(paste("Error with run", x))
                        }
               )
             })
  ...
  if(any(dupd_sams)){
    stop( writeLines(
                ...
                compiled_indices$run_name[dupd_sams])
    )
    )
  }
  ...
  blank_sums = sapply(run_dirs,
                      function(x) read.table(file.path(x,
                                                       "processed_data",
                                                       "blanks",
                                                       "blank_sums.txt")))
  
  # sample sheets
  sample_sheets = lapply(processed_runs, function(x) x$sample_sheet)
  sample_sheets = unlist(sample_sheets, recursive = F)
  ...
  write.csv(compiled_indices, file.path(out_dir,"all_sample_indices.csv"), row.names = F)
  ...
  write.csv(sample_sheets, file.path(out_dir,"all_sample_sheets.csv"), row.names = F)
             
  saveRDS(run_eems, file.path(out_dir,"EEMs.rds"))
  saveRDS(run_blanks, file.path(out_dir, "blanks.rds"))

  return(list(indices = compiled_indices,
              eems = run_eems))
}
